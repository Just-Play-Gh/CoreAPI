...
let UsersService = class UsersService {
    ...
    async paginate(options, searchParams = {}) {
        let userRepository;
        if (searchParams) {
            userRepository = (0, typeorm_1.createQueryBuilder)(user_entity_1.User)
                .where(searchParams)
                .withDeleted();
        }
        else {
            userRepository = (0, typeorm_1.createQueryBuilder)(user_entity_1.User).withDeleted();
        }
        const users = await (0, nestjs_typeorm_paginate_1.paginate)(userRepository, options);
        if (!users['items'])
            throw new common_1.HttpException('No users were found', common_1.HttpStatus.NOT_FOUND);
        return users;
    }
    async findOne(id) {
