...
let AuthenticationService = class AuthenticationService {
    ...
    async sendForgotPasswordOtp(body) {
        try {
            const validDto = await (0, validator_1.validateDto)(new forgot_password_dto_1.SendForgotPasswordOtp(), body);
            if (Object.keys(validDto).length > 0)
                throw new common_1.HttpException(validDto, common_1.HttpStatus.BAD_REQUEST);
    ...
    async sendForgotPasswordEmail(body) {
        try {
            const validDto = await (0, validator_1.validateDto)(new forgot_password_dto_1.SendForgotPasswordEmail(), body);
            if (Object.keys(validDto).length > 0)
                throw new common_1.HttpException(validDto, common_1.HttpStatus.BAD_REQUEST);
